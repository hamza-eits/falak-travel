          ...
          <form action="{{URL('/InvoiceSave')}}" method="post">
            ...
            <div class=" ">
              <div class=" ">
                ...
                <div class="row">
                  <div class="col-6"> <img src="{{ asset('documents/'.$company->Logo) }}" alt="">
                   ...
                   <select name="PartyID" id="PartyID" class="form-select select2 mt-5" required="">
                        ...
                        <option value="{{$value->PartyID}}" {{($value->PartyID== session::get('PartyID')) ? 'selected=selected':'' }}>{{$value->PartyID}}-{{$value->PartyName}}-{{$value->Phone}}
                        </option>
                        <?php endforeach ?>
                      </select>
...
var i = $('table tr').length;
    ...
    $(".addmore").on('click', function() {
        ...
        html += '<select name="SupplierID[]" id="SupplierID_' + i + '" onchange="ajax_balance(this.value);" class="form-select select2" required><option value="">Select</option>@foreach ($supplier as $key => $value) <option value="{{$value->SupplierID}}">{{$value->SupplierName}}</option>@endforeach</td>';
