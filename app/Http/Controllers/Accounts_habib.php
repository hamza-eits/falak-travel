...
// this is normal report wihtout comparison
/*
public function ProfitAndLoss1(Request $request)
{
    ...
    // Helper function to load grouped accounts (R or E)
    $getAccounts = function ($code, $startDate, $endDate) {
        $data = [];

        $level2Accounts = ChartOfAccount::where('CODE', $code)
            ->where('Level', 2)
            ->get();

        foreach ($level2Accounts as $parent) {
            $level3Accounts = ChartOfAccount::where('L2', $parent->ChartOfAccountID)
                ->where('CODE', $code)
                ->where('Level', 3)
                ->whereIn('ChartOfAccountID', function ($query) use ($startDate, $endDate) {
            ...
            foreach ($level3Accounts as $account) {
                $journalTotals = Journal::where('ChartOfAccountID', $account->ChartOfAccountID)
                    ->whereBetween('Date', [$startDate, $endDate])
                    ->selectRaw('COALESCE(SUM(Dr), 0) as total_debit, COALESCE(SUM(Cr), 0) as total_credit')
                    ->first();

                $Dr = $journalTotals->total_debit;
                $Cr = $journalTotals->total_credit;
                $Bal = $code === 'R' ? ($Cr - $Dr) : ($Dr - $Cr); // Reverse logic for expense
            ...
            if (count($children) > 0) {
                $data[] = [
                    'parent_name' => $parent->ChartOfAccountName,
                    'children' => $children,
                ];
            }
        }
    ...
    // Get both sections
    $revenue = $getAccounts('R', $startDate, $endDate);
    $expenses = $getAccounts('E', $startDate, $endDate);

    // Totals
    $totalRevenue = 0;
    foreach ($revenue as $group) {
        foreach ($group['children'] as $child) {
            $totalRevenue += $child['Bal'];
        }
    }
    ...
    return view('reports.profit_and_loss', compact(
        'revenue', 'expenses', 'startDate', 'endDate',
        'totalRevenue', 'totalExpense', 'netProfit'
    ));
}
    */
// month comparison report
public function ProfitAndLoss1_monthly(Request $request)
{
    $startDate = \Carbon\Carbon::parse($request->StartDate ?? now()->startOfYear());
    $endDate   = \Carbon\Carbon::parse($request->EndDate ?? now()->endOfYear());

    // 🔹 Generate all months between the range (e.g., 2024-01, 2024-02, ..., 2025-10)
    $months = [];
    ...
    $sections = ['R' => 'Revenue', 'E' => 'Expenses'];
    $reportData = [];

    foreach ($sections as $code => $sectionTitle) {
        $sectionRows = [];

        // 🔹 Level 2 (Parent Accounts)
        $level2Parents = \App\Models\ChartOfAccount::where('CODE', $code)
            ->where('Level', 2)
            ->get();

        foreach ($level2Parents as $parent) {
            $children = \App\Models\ChartOfAccount::where('L2', $parent->ChartOfAccountID)
                ->where('CODE', $code)
                ->where('Level', 3)
                ->get();
            ...
            foreach ($children as $child) {
                ...
                foreach ($months as $month) {
                    ...
                    $to   = \Carbon\Carbon::parse($month . '-01')->endOfMonth();

                    $journalTotals = \App\Models\Journal::where('ChartOfAccountID', $child->ChartOfAccountID)
                        ->whereBetween('Date', [$from, $to])
                        ->selectRaw('COALESCE(SUM(Dr),0) as total_debit, COALESCE(SUM(Cr),0) as total_credit')
                        ->first();

                    $balance = ($journalTotals->total_credit ?? 0) - ($journalTotals->total_debit ?? 0);

                    // 🔸 For Expenses, invert sign
                    if ($code === 'E') {
                        $balance = -$balance;
                    }
                ...
                // ✅ Include only non-zero total accounts
                if (abs($totalBalance) > 0.0001) {
                    $childRows[] = [
                        'ChartOfAccountID' => $child->ChartOfAccountID,
                        'AccountName'      => $child->ChartOfAccountName,
        ...
        $reportData[$sectionTitle] = $sectionRows;
    }

    // 🔹 Calculate Net Profit/Loss per month
    $netProfit = [];
    foreach ($months as $month) {
        $totalRevenue = 0;
        $totalExpense = 0;

        foreach ($reportData['Revenue'] ?? [] as $parent) {
            foreach ($parent['children'] as $child) {
                $totalRevenue += $child['months'][$month] ?? 0;
            }
        }
        foreach ($reportData['Expenses'] ?? [] as $parent) {
            foreach ($parent['children'] as $child) {
                $totalExpense += $child['months'][$month] ?? 0;
            }
        }

        $netProfit[$month] = $totalRevenue - $totalExpense;
    }

    $netProfit['Total'] = array_sum($netProfit);

    return view('reports.month_comparison', [
...
// this is with comparison year wise
public function ProfitAndLoss1_yearly(Request $request)
{
    $startDate = $request->StartDate ?? now()->startOfYear()->toDateString();
    $endDate   = $request->EndDate ?? now()->endOfYear()->toDateString();
    ...
    foreach ($sections as $code => $sectionTitle) {
        ...
        $level2Parents = ChartOfAccount::where('CODE', $code)
            ->where('Level', 2)
            ->get();

        foreach ($level2Parents as $parent) {
            $children = ChartOfAccount::where('L2', $parent->ChartOfAccountID)
                ->where('CODE', $code)
                ->where('Level', 3)
                ->get();
            ...
            foreach ($children as $child) {
                ...
                foreach ($years as $yr) {
                    $journalTotals = Journal::where('ChartOfAccountID', $child->ChartOfAccountID)
                        ->whereYear('Date', $yr)
                        ->whereBetween('Date', [$startDate, $endDate])
                        ->selectRaw('COALESCE(SUM(Dr),0) as total_debit, COALESCE(SUM(Cr),0) as total_credit')
                        ->first();
                    ...
                    // For expenses, invert the sign (so that both R and E behave consistently)
                    if ($code === 'E') {
                        $balance = -$balance;
                    }

                    $yearBalances[(string)$yr] = $balance;
                    $totalBalance += $balance;
                }

                // ✅ Only add this account if it has any non-zero balance
                if (abs($totalBalance) > 0.0001) {
                    $childRows[] = [
                        'ChartOfAccountID' => $child->ChartOfAccountID,
                        'AccountName'      => $child->ChartOfAccountName,
            ...
            // ✅ Only add parent if it has children with non-zero balance
            if (count($childRows) > 0) {
                $sectionRows[] = [
                    'parent_name' => $parent->ChartOfAccountName,
                    'children'    => $childRows,
    ...
    return view('reports.profit_and_loss_comparison', [
        'reportData' => $reportData,
        'years'      => $years,
        'startDate'  => $startDate,
        'endDate'    => $endDate,
  ...
  public function ExpenseUpdate(request $request)
  {
    ...
    DB::table('expense_detail')->where('ExpenseMasterID', $request->ExpenseMasterID)->delete();

    DB::table('journal')->where('ExpenseMasterID', $request->ExpenseMasterID)->delete();
