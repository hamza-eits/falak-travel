      ...
      public function modelVoucherSave(Request $request)
      {
          ...
          $id= DB::table('voucher_master')->insertGetId($voucher_mst);
            ...
            $voucher_det_dr = array (
              ...
              // 'RefNo' => '',
              'Debit' => $request->input('amount_received') , 
          );
          ...
          $voucher_det_cr = array (
            'VoucherMstID' => $id, 
              ...
              'InvoiceNo' => $request->InvoiceMasterID,
              // 'RefNo' => '',
              'Credit' => $request->input('amount_received') , 
...
$id2 = DB::table('journal')->where('InvoiceMasterID',$request->VHNO)->delete();
...
foreach ($v_cashflow as $key => $value) {
 ...
 $cash = DB::table('v_journal')
            ...
            ->whereIn('ChartOfAccountID',[110101,110201,110202])
...
//110400
 ...
 $date1=date_create(date('Y-m-1'));
$date2=date_create(date('Y-m-d'));
$interval = $date1->diff($date2);
// echo "Difference: " . $interval->days . " days\n";
$avg = ($invoice_summary[0]->Service/$interval->days);
 ...
 // die;
        ...
        return view ('dashboard',compact('pagetitle','v_cashflow','invoice_master','expense','revenue','profit_loss','cash','cash1','exp_chart','party_balance','ticket_register','avg'));
    }
...
return response()->json(['PartyID' => $party, 'PartyName' =>  $request->PartyName, 
                'Phone' =>  $request->Phone]);
...
////////////////////////////END SCRIPT ////////////////////////////////////////////////
 ...
 $invoice_detail = DB::table('v_invoice_detail1')
                  ->select ('SalemanName',DB::raw('count(*) as TotalInvoices'),DB::raw('sum(Fare) as Fare'), DB::raw('sum(Service) as Service'),DB::raw('sum(Total) as Total'),DB::raw('sum(Taxable) as Taxable'),DB::raw('sum(Discount) as Discount'))
                     ->whereBetween('Date',array($request->StartDate,$request->EndDate))
                    ->groupBy('SalemanName')
                    // ->orderBy('Date')
